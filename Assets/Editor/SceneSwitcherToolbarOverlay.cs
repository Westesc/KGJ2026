...
public static class EditorSceneSwitcher
{
    ...
    public static bool ShowAllScenes = false;
    ...
    public static void LoadScenes()
    {
        ...
        {
            // iterate through all scenes added to editor build settings
            foreach (EditorBuildSettingsScene editorScene in EditorBuildSettings.scenes)
            {
                ...
                var sceneAsset = AssetDatabase.LoadAssetAtPath(editorScene.path, typeof(SceneAsset));
...
[EditorToolbarElement(ID, typeof(SceneView))]
public class ShowAllScenesToggle : EditorToolbarToggle
{
    public const string ID = SceneSwitcherToolbarOverlay.OverlayID + "/show-all-scenes-toggle";

    private const string Tooltip = "Show all scenes. Click to toggle visibility of scenes not added to build settings.";

    public ShowAllScenesToggle()
    {
        var content = EditorGUIUtility.TrTextContentWithIcon("", Tooltip, EditorSceneSwitcher.ShowAllScenes ? "scenevis_visible_hover" : "scenevis_hidden_hover");
        text = content.text;
        tooltip = content.tooltip;
        icon = content.image as Texture2D;

        value = EditorSceneSwitcher.ShowAllScenes;
        this.RegisterValueChangedCallback(Toggle);
    }

    private void Toggle(ChangeEvent<bool> evt)
    {
        EditorSceneSwitcher.ShowAllScenes = evt.newValue;
